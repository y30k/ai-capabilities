...
- **Dependency-aware delivery** — use tracker-native relationships so agents only implement unblocked work, then propagate verified tracker-accurate Done/Ready transitions across repository boundaries without editing other repositories.
- **Small, approved slices** — implement one ready story or fix at a time in the current repository, validate and stage it, and apply the tracker's actual completion status without claiming Done before its definition is satisfied.
...
- **Reviewer feedback closure** — process unresolved threads, reply where each concern was raised with what changed and why plus commit/validation evidence, push fixes, resolve fully addressed threads, then paginate all threads and loop on newly arrived feedback before rechecking automation.
...
| 6. Implementation | Implement one approved, unblocked slice at a time. For tracked PRD stories, keep technical changes in the current repo, validate and stage them, apply the tracker-accurate completion status, and move newly unblocked dependents to Ready across repositories only after Done is genuinely satisfied. | `implement-prd-stories`, `develop-feature`, `fix-github-issue`, specialty implementation skills | Slice validates; tracked story changes are staged and the accurate completion status is verified; verified Done triggers the cross-repository readiness sweep, while an intermediate or retained non-Done status records the pending Done condition without unblocking dependents |
...
| 10. PR review remediation | Process unresolved review threads, address triaged in-scope issues, validate, stage and commit, reply with what changed and why plus commit/evidence, push normally, resolve fully addressed threads, then paginate all threads and repeat for newly arrived feedback. A direct request authorizes this default sequence unless a user or repository policy requires an approval checkpoint. | `address-pr-review-comments`, `submit-change-request` | Fixes are validated and bound to the verified updated PR/MR head; the final all-pages closure scan finds zero unresolved threads; required automation for that exact head passed or is explicitly waived |
...
- **Close reviewer feedback in the original thread.** Exclude resolved threads from remediation; for each unresolved concern, post a traceable disposition explaining what changed and why with commit/validation evidence, then resolve it only after the fix or answer is complete and delivered. Finish only after a fresh all-pages scan at the exact PR/MR head finds zero unresolved threads.
...
| Skill | Use for | Typical handoff |
| --- | --- | --- |
| `implement-prd-stories` | Approved, unblocked tracker work implemented in the current repo by default, with validated staging, a tracker-accurate completion transition, and cross-repository dependent-story Ready updates only after verified Done. | Staged story with verified completion status → `check-production-readiness`; verified Done → dependency sweep and Ready queue; intermediate or retained non-Done status → record the pending Done condition |
...
| `address-pr-review-comments` | Unresolved review threads needing triaged in-scope fixes, traceable what/why/commit/validation replies, normal push, verified resolution, and a final paginated closure loop that processes newly arrived threads. A direct request authorizes the end-to-end sequence unless approval is explicitly required. | Zero unresolved threads at the exact updated head → `submit-change-request` monitor mode or another review pass; any blocker → incomplete remediation |
...
- **Implementation gate:** each slice is implemented and validated before moving on; a tracked PRD story also has all story-owned changes staged and a verified tracker-accurate completion transition. Verified Done requires a verified cross-repository sweep that moves every newly unblocked dependent to Ready; an intermediate or retained non-Done status must record the unmet Done condition and must not unblock dependents.
...
- **Review remediation gate:** only unresolved threads enter each remediation worklist; addressed threads have traceable replies and verified resolution; a fresh all-pages closure scan at the provider-verified exact updated PR/MR head finds zero unresolved threads; all affected validation passes on that head; and required automation for the same head passes or is explicitly waived.
...
8. On local technical implementation, stage the story changes and verify the tracker-accurate completion transition. Move newly unblocked dependency-linked stories to Ready regardless of owning repository only after the active story genuinely satisfies Done; otherwise record the pending completion condition and skip propagation.
