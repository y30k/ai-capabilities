...
| 10. PR review remediation | Process only unresolved review threads, address triaged in-scope issues, validate, stage and commit, reply with what changed and why plus commit/evidence, push normally, resolve fully addressed threads, and recheck automation. A direct request authorizes this default sequence unless a user or repository policy requires an approval checkpoint. | `address-pr-review-comments`, `submit-change-request` | Fixes are validated and bound to the verified updated PR/MR head; addressed threads are resolved; required automation for that exact head passed or is explicitly waived |
...
| `address-pr-review-comments` | Unresolved review threads needing triaged in-scope fixes, traceable what/why/commit/validation replies, normal push, verified resolution when fully addressed, and CI/CD recheck. A direct request authorizes the end-to-end sequence unless approval is explicitly required. | Resolved addressed threads and updated branch → `submit-change-request` monitor mode or another review pass |
