...
| 10. PR review remediation | Process only unresolved threads; address triaged in-scope feedback; validate, stage, commit, reply, push normally, and resolve fully addressed threads without another approval pause unless the user or repository policy explicitly requires one | `address-pr-review-comments`, `submit-change-request` | Fixes are validated and bound to the verified updated PR/MR head; addressed threads are resolved; required automation for that exact head passed or is explicitly waived |
