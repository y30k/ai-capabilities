...
| 6. Implementation | Code one approved, unblocked slice at a time; for tracked PRD stories, stay in the current repo, stage validated changes, apply the tracker-accurate completion status, and refresh dependent Ready states across repositories only after Done is genuinely satisfied | `implement-prd-stories`, `develop-feature`, or `fix-github-issue` | Slice passes validation; tracked story changes are staged and the accurate completion status is verified; verified Done triggers dependency readiness propagation, while an intermediate or retained non-Done status records the pending Done condition without unblocking dependents |
...
| 10. PR review remediation | Process unresolved threads; address triaged in-scope feedback; validate, stage, commit, reply, push normally, resolve fully addressed threads, then paginate all threads and repeat for newly arrived feedback without another approval pause unless explicitly required | `address-pr-review-comments`, `submit-change-request` | Fixes are validated and bound to the verified updated PR/MR head; the final all-pages closure scan finds zero unresolved threads; required automation for that exact head passed or is explicitly waived |
...
- **Implementation gate**: each slice validates before moving on; a tracked PRD story also has all story-owned changes staged and a verified tracker-accurate completion transition. Verified Done requires a verified cross-repository sweep that moves every newly unblocked dependent to Ready; an intermediate or retained non-Done status must record the unmet Done condition and must not unblock dependents.
